"""
explore_kaggle_dataset.py
--------------------------
Explores the ArXiv Kaggle dataset and saves column metadata to disk.

FIX (#22): Output directory 'Dataset/' previously had to exist manually.
Now os.makedirs() is called with exist_ok=True before writing any files
so the script works even on a fresh checkout with no Dataset/ folder.
"""
...
# ─────────────────────────────────────────────────────────────
# FIX (#22): Ensure output directory exists before writing files
# ─────────────────────────────────────────────────────────────
OUTPUT_DIR = 'Dataset'
...
if not data_files:
    ...
    for file_path, file_name, file_ext, file_size in data_files:
        ...
        try:
            ...
            # FIX (#22): OUTPUT_DIR is guaranteed to exist
            output_json = os.path.join(OUTPUT_DIR, 'arxiv_dataset_columns.json')
            with open(output_json, 'w', encoding='utf-8') as f:
                json.dump(col_info, f, indent=2, default=str)
            print(f"\n   [OK] Column information saved to '{output_json}'")
            ...
            output_csv = os.path.join(OUTPUT_DIR, 'arxiv_dataset_columns.csv')
            col_df.to_csv(output_csv, index=False)
            print(f"   [OK] Column list saved to '{output_csv}'")

            print(f"\n   [OK] Successfully analysed: {file_name}")
            break
